...
function parseWorkbook(bytes) {
  ...
  const columns = [];
  for (let i = 1; i < header.length; i++) { const observationDate = observationMonth(header[i]); if (observationDate) columns.push({ index: i, observationDate }); }
  ...
  for (let r = 3; r < matrix.length; r++) {
    ...
    if (!rel) continue;
    const rows = [];
    for (const col of columns) { const value = numeric(row[col.index]); if (value !== null) rows.push({ observationDate: col.observationDate, releaseDate: rel, value }); }
...
for (const release of releases) {
  for (let start = 0; start < release.rows.length; start += MAX_ROWS_PER_REQUEST) {
    ...
    requests += 1; totalRows += chunk.length;
  }
