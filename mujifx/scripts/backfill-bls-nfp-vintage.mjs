...
function releaseDate(value) { if (value instanceof Date && !Number.isNaN(value.getTime())) return value.toISOString().slice(0, 10); const parsed = new Date(String(value ?? "").trim()); if (Number.isNaN(parsed.getTime())) return null; const iso = parsed.toISOString().slice(0, 10); return /^20\d{2}-\d{2}-\d{2}$/.test(iso) ? iso : null; }
...
function observationAvailableByRelease(observationDate, releaseDate) { const [year, month] = observationDate.split("-").map(Number); const end = new Date(Date.UTC(year, month, 0)); return new Date(`${releaseDate}T00:00:00Z`) > end; }
function parseWorkbook(bytes) {
  const wb = XLSX.read(bytes, { type: "buffer", cellDates: true, raw: false }); const sheet = wb.Sheets.Data; if (!sheet) throw new Error("BLS workbook is missing Data sheet.");
  const matrix = XLSX.utils.sheet_to_json(sheet, { header: 1, raw: false, defval: null }); let headerRowIndex = -1; let columns = [];
  for (let rowIndex = 0; rowIndex < Math.min(matrix.length, 30); rowIndex++) { const row = matrix[rowIndex] ?? []; const candidate = []; for (let i = 1; i < row.length; i++) { const obs = observationMonth(row[i]); if (obs) candidate.push({ index: i, observationDate: obs }); } if (candidate.length > columns.length) { columns = candidate; headerRowIndex = rowIndex; } }
  if (headerRowIndex < 0 || !columns.length) throw new Error("No observation-month columns found.");
  const releases = new Map();
  for (let r = headerRowIndex + 1; r < matrix.length; r++) { const row = matrix[r] ?? []; const rel = releaseDate(row[0]); if (!rel) continue; const rows = []; for (const col of columns) { const value = numeric(row[col.index]); if (value !== null && observationAvailableByRelease(col.observationDate, rel)) rows.push({ observationDate: col.observationDate, releaseDate: rel, value }); } if (rows.length) releases.set(rel, rows); }
  return [...releases.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([releaseDate, rows]) => ({ releaseDate, rows }));
}
async function publish(rows, publicationDate, releaseDate) { const snapshot = { snapshotUrl: BLS_URL, publicationDate, label: "BLS CES Total Nonfarm Vintage Data" }; const response = await fetch(VERCEL_URL, { method: "POST", headers: { Authorization: `Bearer ${SECRET}`, "Content-Type": "application/json" }, body: JSON.stringify({ kind: "NFP", snapshot, rows }) }); const text = await response.text(); if (!response.ok) throw new Error(`Release ${releaseDate} failed: HTTP ${response.status}: ${text}`); return JSON.parse(text); }
...
if (!response.ok) throw new Error(`BLS vintage download failed with HTTP ${response.status}.`); const lastModified = response.headers.get("last-modified"); if (!lastModified) throw new Error("BLS vintage download did not provide Last-Modified provenance."); const publication = new Date(lastModified); if (Number.isNaN(publication.getTime())) throw new Error(`Invalid BLS Last-Modified: ${lastModified}`);
const publicationDate = publication.toISOString().slice(0, 10); const releases = parseWorkbook(Buffer.from(await response.arrayBuffer())); let totalRows = 0; let requests = 0;
